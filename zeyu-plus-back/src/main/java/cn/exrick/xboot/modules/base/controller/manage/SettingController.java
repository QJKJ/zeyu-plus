...
public class SettingController {
    ...
    @RequestMapping(value = "/core/set",method = RequestMethod.POST)
    @ApiOperation(value = "核心配置")
    public Result<Object> coreSet(@ModelAttribute CoreSetting coreSetting) {
        System.out.print(coreSetting+"<<<");
        String v = redisTemplate.opsForValue().get(SettingConstant.CORE_SETTING);
        redisTemplate.opsForValue().set(SettingConstant.CORE_SETTING, new Gson().toJson(coreSetting));
        return new ResultUtil<Object>().setData(null);
    }


    @RequestMapping(value = "/core",method = RequestMethod.GET)
    @ApiOperation(value = "查看核心配置")
    public Result<CoreSetting> core() {

        String v = redisTemplate.opsForValue().get(SettingConstant.CORE_SETTING);
        if(StrUtil.isBlank(v)){
            return new ResultUtil<CoreSetting>().setData(null);
        }
        CoreSetting coreSetting = new Gson().fromJson(v, CoreSetting.class);
        return new ResultUtil<CoreSetting>().setData(coreSetting);
    }
